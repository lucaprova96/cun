...
import com.luce.extractors.VixcloudExtractor
...
import com.luce.utils.InertiaUtils
import com.luce.utils.NetworkClient
import com.luce.utils.TmdbUtils
...
class StreamingCommunityProvider(private val _language: String? = null) : Provider {
    ...
    override suspend fun getServers(id: String, videoType: Video.Type): List<Video.Server> {
        Log.i("LuceES", "[PROV] -> StreamingCommunity: getServers for $id")
        ...
        Log.i("LuceES", "[PROV] -> Found Vixcloud src: $src")
        return listOf(Video.Server(id = id, name = "Vixcloud", src = src))
    }

    override suspend fun getVideo(server: Video.Server): Video {
        Log.i("LuceES", "[SERVER] -> StreamingCommunity: Using ${server.name} (Src: ${server.src})")
