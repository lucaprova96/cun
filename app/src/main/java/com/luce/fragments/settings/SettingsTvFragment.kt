package com.luce.fragments.settings
...
import com.luce.R
import com.luce.activities.main.MainTvActivity
import com.luce.backup.BackupRestoreManager
import com.luce.backup.ProviderBackupContext
import com.luce.database.AppDatabase
import com.luce.database.dao.EpisodeDao
import com.luce.database.dao.MovieDao
import com.luce.database.dao.TvShowDao
import com.luce.database.dao.SeasonDao
import com.luce.providers.FrenchStreamProvider
import com.luce.providers.Provider
import com.luce.providers.ProviderConfigUrl
import com.luce.providers.ProviderPortalUrl
import com.luce.providers.StreamingCommunityProvider
import com.luce.providers.TmdbProvider
import com.luce.utils.BypassWebSocketEndpointHelper
import com.luce.utils.AppLanguageManager
import com.luce.utils.DnsResolver
import com.luce.utils.ProviderChangeNotifier
import com.luce.utils.QrUtils
import com.luce.utils.ThemeManager
import com.luce.utils.UserDataCache
import com.luce.utils.UserPreferences
import com.luce.utils.WebSocketBypassTestHelper
    ...
    private fun displaySettings() {
        ...
        findPreference<Preference>("p_settings_help")?.setOnPreferenceClickListener {
            startActivity(
                Intent(
                    Intent.ACTION_VIEW,
                    Uri.parse("https://github.com/Luce-reborn/Luce")
                )
            )
            true
        }
        ...
        findPreference<Preference>("p_settings_telegram")?.setOnPreferenceClickListener {
            try {
                val tgIntent = Intent(Intent.ACTION_VIEW, Uri.parse("tg://resolve?domain=Lucereborn"))
                startActivity(tgIntent)
            } catch (e: Exception) {
                Toast.makeText(requireContext(), "Telegram not found.", Toast.LENGTH_SHORT).show()
                val intent = Intent(Intent.ACTION_VIEW, Uri.parse("https://t.me/Lucereborn"))
        ...
        findPreference<Preference>("key_backup_export_tv")?.setOnPreferenceClickListener {
            ...
            val fileName = "Luce_tv_backup_$timestamp.json"
        ...
        findPreference<Preference>("key_backup_export_db_tv")?.setOnPreferenceClickListener {
            val timestamp = SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault()).format(Date())
            val fileName = "Luce_tv_db_backup_$timestamp.zip"
            ...
            withBackupLoading(R.string.backup_export_title) {
                ...
                runCatching {
                    val values = ContentValues().apply {
                        ...
                        put(MediaStore.Downloads.RELATIVE_PATH, Environment.DIRECTORY_DOWNLOADS + "/Luce")
                    }
                    ...
                    Toast.makeText(
                        requireContext(),
                        getString(R.string.backup_export_saved_to, "Downloads/Luce/$fileName"),
                        Toast.LENGTH_LONG
                    ).show()
                }.onFailure { error ->
                    Toast.makeText(requireContext(), getString(R.string.backup_export_error_write), Toast.LENGTH_LONG).show()
