        ...
        try {
            ...
            client.newCall(request).execute().use { response ->
                if (response.isSuccessful && response.body != null) {
                    ...
                    Log.i("LuceES", "[VixSrc] --- Processing START (Lang: $langCode) ---")
                    ...
                    for (line in lines) {
                        ...
                        if (patchedLine.startsWith("#EXT-X-MEDIA:TYPE=AUDIO")) {
                            ...
                            if (isTargetAudio) {
                                ...
                                Log.i("LuceES", "[AUDIO] -> SET DEFAULT: $langCode")
                            }
                            ...
                            if (isForced && isRightLanguage) {
                                patchedLine = patchedLine.replace("DEFAULT=NO", "DEFAULT=YES")
                                                         .replace("AUTOSELECT=NO", "AUTOSELECT=YES")
                                Log.i("LuceES", "[SUBTITLE] -> ENABLED FORCED: $trackName")
                            }
                            finalLines.add(patchedLine)
                        } else {
                            finalLines.add(patchedLine)
                    ...
                    Log.i("LuceES", "[VixSrc] --- Processing END ---")
